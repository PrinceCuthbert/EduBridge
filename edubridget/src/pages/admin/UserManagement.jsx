...
import { Search, Filter, Plus, Users, UserCircle, Shield, Edit, Trash2, Eye, X, ChevronDown, Download } from 'lucide-react';
import { toast } from 'sonner';
import Modal from '../../components/Modal';

// Mock data moved outside component
const MOCK_USERS = [
  { id: 1, name: 'Alice Mutesi', email: 'alice@example.com', role: 'Student', phone: '+250 788 123 456', country: 'Rwanda', status: 'Active', joined: '2024-01-15' },
  { id: 2, name: 'David Kwizera', email: 'david@example.com', role: 'Admin', phone: '+250 788 654 321', country: 'Rwanda', status: 'Active', joined: '2024-01-10' },
  { id: 3, name: 'Sarah Uwase', email: 'sarah@example.com', role: 'Student', phone: '+250 788 987 654', country: 'Uganda', status: 'Inactive', joined: '2024-01-20' },
];
...
export default function UserManagement() {
  ...
  const stats = [
    ...
    { label: 'Students', value: users.filter(u => u.role === 'Student').length, icon: UserCircle, color: 'text-green-600', bg: 'bg-green-50' },
    { label: 'Admins', value: users.filter(u => u.role === 'Admin').length, icon: Shield, color: 'text-purple-600', bg: 'bg-purple-50' },
  ];
        ...
        <div className="p-6 border-b border-slate-200">
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            ...
            >
              <Filter size={18} />
              Filters
              {(filters.role !== 'All' || filters.status !== 'All' || filters.country !== 'All') && (
            ...
            <div className="relative">
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg border border-slate-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-10">
                <button onClick={() => handleExport('csv')} className="w-full text-left px-4 py-2 text-sm hover:bg-slate-50">Export as CSV</button>
                <button onClick={() => handleExport('excel')} className="w-full text-left px-4 py-2 text-sm hover:bg-slate-50">Export as Excel</button>
                <button onClick={() => handleExport('api')} className="w-full text-left px-4 py-2 text-sm hover:bg-slate-50">Export All (API)</button>
              </div>
            </div>
            <button 
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <span className={`inline-block mt-2 px-3 py-1 rounded-full text-xs font-semibold ${getRoleBadgeColor(selectedUser.role)}`}>
                  {selectedUser.role}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p className="font-medium text-slate-900">{selectedUser.country}</p>
              </div>
              <div>
                <p className="text-sm text-slate-500 mb-1">Status</p>
                <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold ${getStatusColor(selectedUser.status)}`}>
                  {selectedUser.status}
                </span>
              </div>
              <div>
