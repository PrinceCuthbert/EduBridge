...
const SectionHeader = ({ icon: Icon, title, subtitle }) => (
  <div className="mb-5">
    ...
    {subtitle && (
      <p className="text-[11px] text-slate-400 font-medium tracking-wider uppercase ml-9">
        {subtitle}
      </p>
    )}
  </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="px-4 py-3 text-left text-[11px] font-semibold text-slate-500 uppercase tracking-wider w-44">
                Lang. Req.
              </th>
            </tr>
          </thead>
        ...
        {departments.map((dept, idx) => (
          <div
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 pt-1">
              <div>
                <span className="block font-medium text-slate-400 uppercase tracking-wider text-[10px] mb-0.5">
                  Duration
                </span>
                {dept.duration}
                {dept.credits ? ` — ${dept.credits}` : ""}
              </div>
              <div>
                <span className="block font-medium text-slate-400 uppercase tracking-wider text-[10px] mb-0.5">
                  Language
          ...
          <div
            ...
            <div className="grid grid-cols-1 gap-2 text-sm">
              <div className="flex justify-between">
                <span className="text-slate-400 text-xs uppercase font-medium tracking-wider self-center">
                  Registration
                </span>
                <span className="text-slate-700 font-medium text-right">
                  {fmt(item.registrationStart)}
                  {item.registrationEnd
                    ? ` → ${fmt(item.registrationEnd)}`
                    : ""}
                </span>
              </div>
      ...
      <div className="flex items-start gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Please review all requirements above before submitting your
            application.
          </p>
        </div>
      </div>
